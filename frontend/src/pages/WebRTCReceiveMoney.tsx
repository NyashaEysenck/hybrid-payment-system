...
interface Transaction {
  id: string; // This is the receiver's internally generated receiptId
  type: 'receive'; //
  amount: number; //
  sender: string; //
  recipient: string; //
  ...
  receiptId: string; // Typically same as 'id' for receiver transactions
  synced?: boolean; //
  senderTransactionId?: string; // Add this to store the sender's original transaction ID
}
...
const WebRTCReceiveMoney = () => {
  ...
  const handleQrCodeScanned = async (data: string) => {
    ...
    try {
      ...
      // Single QR code logic (non-chunked)
      try {
  ...
  const handlePaymentReceived = async (paymentData: any) => {
    ...
    let receiptId = ''; //
    let localPendingTransaction: Transaction | null = null; // Use a different name to avoid confusion with state 'transaction'
    ...
    try {
      ...
      const alreadyCompletedTx = existingTransactions.find(
        (tx): tx is Transaction => // Type guard for TS
          tx.type === 'receive' &&
          (tx as any).senderTransactionId === paymentData.transactionId && // Check against sender's transaction ID
          tx.status === 'completed'
      );

      if (alreadyCompletedTx) {
        console.log(`Transaction ${paymentData.transactionId} (sender) already completed with our receipt ID ${alreadyCompletedTx.receiptId}. Resending receipt.`);
        ...
        setTransaction(alreadyCompletedTx); // Update UI to show this existing completed transaction
        setStep('complete');
        toast({
          title: "Payment Already Processed",
          description: `Payment from ${alreadyCompletedTx.sender} for $${alreadyCompletedTx.amount.toFixed(2)} was already recorded.`,
          duration: 5000,
        });
      ...
      const newPendingTransaction: Transaction = {
        ...
        synced: false, //
        senderTransactionId: paymentData.transactionId, // Store sender's transaction ID
      };
      localPendingTransaction = newPendingTransaction; //
      setTransaction(localPendingTransaction); //
      await storageService.saveTransaction(localPendingTransaction); //
      ...
      console.log('Sent receipt:', { //
        receiptId: localPendingTransaction.receiptId,
        transactionId: paymentData.transactionId,
        status: 'success',
      });

      const completedTransaction: Transaction = { //
        ...localPendingTransaction, //
      ...
      if (localPendingTransaction) { // Check the locally scoped variable
      ...
      } else if (transaction) { // Fallback to state variable if local one wasn't set (should ideally not happen in this flow)
         const failedTransactionFromState: Transaction = { //
          ...transaction, //
          status: 'failed' as const, //
        };
        setTransaction(failedTransactionFromState); //
        await storageService.saveTransaction(failedTransactionFromState); //
      }

      try {
        await webrtcService?.sendMessage({ //
          type: 'receipt',
          receiptId: receiptId, // This might be empty if error occurred before its generation
          status: 'failed',
          error: errorMessage,
          transactionId: paymentData.transactionId,
        });
          ...
          {step === 'waitForPayment' && (
            <div className="space-y-6 text-center"> {/* */}
              ...
              <p className="text-sm text-gray-500"> {/* */}
                ...
                Waiting for the sender to complete the payment... {/* */}
              </p>
              ...
              <Card> {/* */}
                ...
                <CardContent className="space-y-4"> {/* */}
                  ...
                  {transaction.senderTransactionId && (
                     <div className="pt-2">
                        <span className="text-gray-500 block mb-1">Sender Tx ID</span>
                        <p className="bg-gray-50 p-2 rounded text-xs font-mono break-all">
                          {transaction.senderTransactionId}
                        </p>
                     </div>
